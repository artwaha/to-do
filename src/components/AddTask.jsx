...
const AddTask = () => {
    const { tasks, userId, updateTasks } = useContext(TodoContext)
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.post('http://localhost:3001/tasks/count-tasks', { userId })
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:3001/tasks/new-task', { newTask, userId });
