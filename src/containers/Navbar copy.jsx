...
export default function Navbar() {
  const navigation = [
    ...
    { name: "Todo", path: "todo" },
    { name: "Done", path: "done" },
    { name: "Collaborating", path: "collaborating" },
          ...
          <ul className="items-center justify-end flex-1 pt-6 lg:pt-0 list-reset lg:flex">
            {navigation.map((menu, index) => (
              <li className="mr-3 nav__item" key={index}>
                <Link
                  to={menu.path}
                  className="inline-block p-2 font-normal text-gray-800 no-underline rounded-md  hover:text-indigo-500 focus:text-indigo-500 focus:bg-indigo-100 focus:outline-none"
                >
                  {menu.name}
                </Link>
              </li>
